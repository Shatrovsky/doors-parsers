...
class BelwoodDoors extends Command
{
     ...
     * @var string
     */
    protected $signature = 'parsing:belwooddoors';
    ...
    private array $modelUrls = ['https://belwooddoors.ru/catalog/mezhkomnatnye_dveri/alba-osteklennoe-raspashnaya/'];
    ...
    {
        $this->file = fopen('belwooddoors.csv', 'w');
        fputcsv($this->file, BelwoodDoorsProduct::$headers, "\t");
        foreach ($this->urls as $url) {
            $pageUrls = $this->getPageUrls($url);
            foreach ($pageUrls as $pageUrl){
                $this->getModelUrls($pageUrl);
            }
            $total = 1;
            $errorCount = 0;
